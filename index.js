/*
]=====> NURULCANS <=====[ ]=====> MANCABOT <=====[ ]=====> 083815956151 <=====[
*Hayoo mau ngapain kesini?
*edit yaaa? izin dulu brow wa Ada Di Atas;)
*/
const {
   WAConnection,
   MessageType,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' NURUL CANS'))
})
	...
	client.on('connecting', () => {
		start('2', 'Bot Connecting...')
	})
	client.on('open', () => {
		success('2', 'Bot Connected')
	})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `╭❑➢─|≪━𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝙶𝙲━≫|─➢° ᭄\n╎᭄\n╎❑➢ ${mdata.subject}\n╎__________________________________\n╎᭄\n╎@${num.split('@')[0]} Intro/Dikick!!! \n╎╎\n╎❒► 𝙽𝙰𝙼𝙰 : \n╎╎\n╎❒► 𝚄𝙼𝚄𝚁 : \n╎╎\n╎❒► 𝙰𝚂𝙺𝙾𝚃 : \n╎╎\n╎❒► 𝙶𝙴𝙽𝙳𝙴𝚁 : \n╎╎\n╎❒► 𝚄𝙳𝙰𝙷 𝙿𝚄𝙽𝚈𝙰 𝙳𝙾𝙸? : \n╎╎\n╎❒► 𝙰𝚂𝚂𝙰𝙻𝙰𝙼𝚄𝙰𝙻𝙰𝙸𝙺𝚄𝙼 𝙳𝚄𝙻𝚄 : \n╎╎\n╎❒► 𝙸𝙽𝚂𝚃𝙰𝙶𝚁𝙰𝙼? : \n╎╎\n╎╎᭄\n╎᭄╰❑➢ 𝚂𝚅 𝚆𝙰 𝙰𝙳𝙼𝙸𝙽\n╎❑➢  𝙹𝙰𝙽𝙶𝙰𝙽 𝙻𝚄𝙿𝙰 𝚂𝙾𝙿𝙰𝙽 𝚈𝙰𝙷 𝚂𝙰𝚈𝙰𝙽𝙶 \n╎᭄\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`
				...
				teks = `╭❑➢─|≪━𝚂𝙴𝙻𝙰𝙼𝙰𝚃 𝚃𝙸𝙽𝙶𝙶𝙰𝙻━≫|─➢° ᭄\n╎ \n╎❑➢ @${num.split('@')[0]}👋 \n╎᭄\n╎╭❑➢─|≪━━━━━━━━━━≫|─➢° ᭄\n╎╎᭄\n╎╎𝙹𝚊𝚜𝚊𝚖𝚞 𝚊𝚔𝚊𝚗 𝚜𝚊𝚢𝚊 𝚔𝚞𝚋𝚞𝚛 𝚍𝚊𝚕𝚊𝚖 𝚍𝚊𝚕𝚊𝚖\n╎╎᭄\n╎╎𝙺𝚊𝚕𝚘 𝙶𝚊 𝙽𝚒𝚊𝚝 𝙼𝚊𝚜𝚞𝚔 𝙶𝚛𝚞𝚙\n╎╎᭄\n╎╎𝙹𝚊𝚗𝚐𝚊𝚗 𝙼𝚊𝚜𝚞𝚔!!\n╎╎᭄\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
switch(command) {
				...
				if (!isRegistered) return reply(ind.noregis())
					...
					const lolican= fs.readFileSync('./assets/lolican.mp3');
					client.sendMessage(from, lolican, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					break
				case 'donasi':
				case 'donate':
				...
				if (!isRegistered) return reply(ind.noregis())			
					...
					const lolican= fs.readFileSync('./assets/lolican');
					client.sendMessage(from, lolican, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
					break
                case 'makermenu':
                if (isBanned) return reply(ind.baned())
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis1 Nurul baik hati')
				ramlan = body.slice(8)
				reply('「❗」WAIT KAK  NGAK LAMA KOK :V')
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply('Teksnya mana kak? Contoh : ${prefix}nulis2 Nurul baik hati')
				laysha = body.slice(8)
				reply('「❗」WAIT KAK NGAK LAMA KOK :V')
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					teks = body.slice(7)
					reply('「❗」WAIT KAK GUE NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : NURUL CANS._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}ninjalogo Nurul & Cans')
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply('「❗」Contoh : ${prefix}pornhub FxSx & Hub')
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}gemboktext 11 01 2021 & Nurul dan FxSx')
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply('[❗] Contoh : ${prefix}glitchtext Nurul & Cans')
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, Sayang?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata Nurul`
...
*/
           ...
           if (!isRegistered) return reply(ind.noregis())
					apakah = body.slice(1)
					const apa = fs.readFileSync('./assets/iya/mungkin tidak/mungkin iya/tidak/ferguso');
...
*/		 
                ...
                resul = `╭❑➢─|≪━𝙻𝙴𝚅𝙴𝙻━≫|─➢° ᭄\n╎\n╎❑➢ 𝙽𝙰𝙼𝙰 : ${pushname}\n╎\n╎❑➢ 𝙽𝙾𝙼𝙾𝚁 : wa.me/${sender.split("@")[0]}\n╎\n╎❑➢ 𝚇𝙿 : ${userXp}/${requiredXp}\n╎\n╎❑➢ 𝙻𝙴𝚅𝙴𝙻 : ${userLevel}\n╎\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`
...
*/
                ...
                reply(`╭❑➢─|≪━𝚂𝚄𝙺𝚂𝙴𝚂━≫|─➢° ᭄\n╎\n╎❒► 𝙿𝚎𝚗𝚐𝚒𝚛𝚒𝚖𝚊𝚗 𝚄𝚊𝚗𝚐 𝙱𝚎𝚛𝚑𝚊𝚜𝚒𝚕\n╎\n╎❒► 𝙳𝚊𝚛𝚒 : +${sender.split("@")[0]}\n╎❑► 𝙺𝚎 : +${tujuan}\n╎❒► 𝙹𝚞𝚖𝚕𝚊𝚑 𝚃𝚛𝚊𝚗𝚜𝚏𝚎𝚛 : ${jumblah}\n╎❑► 𝙿𝚊𝚓𝚊𝚔 : ${fee}\n╎\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`)
                break
				case 'atm':
                  if (isBanned) return reply(ind.baned())				
				if (!isRegistered) return reply(ind.noregis())
				...
				if ( checkATMuser(sender) <= total) return reply(`maaf kak uang nya gak cukup, kumpulin uang nya dulu >_< kak:v`)
				if ( checkATMuser(sender) >= total ) {
					confirmATM(sender, total)
					bayarLimit(sender, payout)
					await reply(`╭❑➢─| 𝙱𝙰𝚈𝙰𝚁 ⊱✿⊰ 𝙱𝙴𝚁𝙷𝙰𝚂𝙸𝙻 |─➢° ᭄\n╎\n╎❒► 𝙿𝚎𝚗𝚐𝚒𝚛𝚒𝚖 : 𝙽𝚄𝚁𝚄𝙻\n╎❑► 𝙿𝚎𝚗𝚎𝚛𝚒𝚖𝚊 : ${pushname}\n╎❒► 𝙽𝚘𝚖𝚒𝚗𝚊𝚕 𝙿𝚎𝚖𝚋𝚎𝚕𝚒𝚊𝚗 : ${payout}\n╎❑► 𝙷𝚊𝚛𝚐𝚊 𝙻𝚒𝚖𝚒𝚝 : ${koinPerlimit}/limit\n╎❒► 𝚂𝚒𝚜𝚊 𝚄𝚊𝚗𝚐 : ${checkATMuser(sender)}\n╎❑► 𝚙𝚛𝚘𝚜𝚎𝚜 𝚋𝚎𝚛𝚑𝚊𝚜𝚒𝚕 𝚍𝚎𝚗𝚐𝚊𝚗 𝚂𝙽\n╎❒► ${createSerial(15)}\n╎\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`)
				} 
				break
/*
]=====> RANDOM MENU <=====[
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner Nurul`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena Nurul baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'next':
				...
				case 'info':
					...
					teks = `╭❑➢─|≪━━━𝙸𝙽𝙵𝙾━━━≫|─➢° ᭄\n╎\n╎❒► 𝙽𝚊𝚖𝚎 𝙱𝚘𝚝 : ${me.name}\n╎❒► 𝙾𝚠𝚗𝚎𝚛 : 𝙽𝚄𝚁𝚄𝙻\n╎❒► 𝙰𝚞𝚝𝚑𝚘𝚛 : ${ownerName}\n╎❒► 𝙽𝚘𝚖𝚎𝚛 𝙱𝚘𝚝 : @${me.jid.split('@')[0]}\n╎❒► 𝙿𝚛𝚎𝚏𝚒𝚡 : ${prefix}\n╎❒► 𝚃𝚘𝚝𝚊𝚕 𝙱𝚕𝚘𝚌𝚔 𝙲𝚘𝚗𝚝𝚊𝚌𝚝 : ${blocked.length}\n╎❒► 𝚃𝚑𝚎 𝙱𝚘𝚝 𝙸𝚜 𝙰𝚌𝚝𝚒𝚟𝚎 𝙾𝚗 : ${kyun(uptime)}\n╎\n╰❑➢─| 𝙼𝙰𝙽𝙲𝙰 ⊱✿⊰ 𝙱𝙾𝚃 |─➢°࿐`
...
har = body.slice(12)
reply('「❗」HARTA TAHTA TETE KARINA :V')
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply('「❗」Contoh : ${prefix}cloudtext Nurul')
cloud = body.slice(11)
reply('「❗」Bentar Bro Terbang dulu yakan')
...
break
case 'manca':
const manca = fs.readFileSync('./assets/manca.mp3');
client.sendMessage(from, manca, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'slow':
...
break
case 'terpesona':
const sona = fs.readFileSync('./assets/terpesona.mp3');
client.sendMessage(from, sona, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'lolican':
const lolican= fs.readFileSync('./assets/lolican.mp3');
client.sendMessage(from, lolican, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
